...
models.User = B.model('User', {
  ...
  async Patch_ResetPassword(ctx) {
    ...
    await Q('user').update('password', '123456').where({
      id
    })
    throw "密码重置成功"
  },
  ...
  async Patch(ctx) {
    ...
    await B.transaction(t => {
      ...
      if (deps) {
        ...
        promises.push(Q('dep_user').transacting(t).where({
          user_id: id
        }).del())
        promises.push(Q('dep_user').transacting(t).insert(params))
      }
      ...
      if (Object.keys(data).length > 0)
        promises.push(Q('user').transacting(t).update(data).where({
          id
        }))
  ...
  async Post(ctx) {
    ...
    return model.toJSON({
      visible: ['id', 'created_by', 'created_at']
    })
  }
})
